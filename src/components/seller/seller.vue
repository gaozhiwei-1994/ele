...
<script>
  ...
  export default {
    ...
    created () {// 从其它路由切换过来,此时seller数据已经加载完成，通过标签属性传递过来
      this.classMap = ['decrease', 'discount', 'special', 'invoice', 'guarantee']
      console.log('c',this.seller.name)
      if(this.seller.name){//保证只会创建一个scroll对象
        console.log('jin-lai-le')
        this._initScroll()
      }

    },
    updated () {//在当前路由刷新 seller变化了，此时created中seller属性还没有数据，
                //所以在这里创建scroll对象
      console.log('u',this.seller.name)
      this._initScroll()
    },
    methods: {
      //切换收藏状态的方法
      ...
      _initScroll () {
        this.$nextTick(()=>{
          ...
          const ulWidth = liCount * (space + liWidth) - space
